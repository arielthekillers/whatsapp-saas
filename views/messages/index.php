...
$flashError = $error ?? $_SESSION['flash_error'] ?? null;
...
<div>
  ...
  <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
    <div>
      <h1 class="text-2xl sm:text-3xl font-extrabold text-gray-900 tracking-tight font-display">Pesan &amp; Riwayat Logs
      </h1>
      <p class="text-sm text-gray-500 mt-1">Kirim pesan WhatsApp langsung dan pantau log riwayat pesan per sesi dengan
        mudah.</p>
    </div>
    ...
    <div class="flex items-center gap-3">
      <div
        class="px-3.5 py-2 bg-white border border-gray-200 rounded-xl text-xs font-semibold text-gray-700 flex items-center gap-2 shadow-xs">
  ...
  <?php if ($flashSuccess): ?>
    <div
      class="mb-6 p-4 bg-emerald-50 border border-emerald-200 text-emerald-800 text-xs rounded-xl flex items-center gap-3 shadow-xs font-medium">
  ...
  <?php if ($flashError): ?>
    <div
      class="mb-6 p-4 bg-rose-50 border border-rose-200 text-rose-800 text-xs rounded-xl flex items-center gap-3 shadow-xs font-medium">
        ...
        <?php if (empty($sessions)): ?>
          <div class="p-4 bg-amber-50 border border-amber-200 rounded-xl text-center">
            ...
            <a href="<?= url('/sessions/create') ?>"
              class="inline-flex items-center gap-1.5 text-xs font-bold text-purple-700 hover:underline">
              + Buat Sesi Baru
            </a>
          </div>
        <?php else: ?>
          <form method="POST" action="<?= url('/messages/send') ?>" class="space-y-4">
            ...
            <!-- Session Selector -->
            <div>
              <label class="block text-xs font-semibold text-gray-700 mb-1.5">Pilih Sesi WhatsApp</label>
              <select name="session_id" required
                class="w-full bg-gray-50 border border-gray-200 rounded-xl px-3 py-2 text-xs font-medium text-gray-800 focus:outline-none focus:ring-2 focus:ring-purple-500/20 focus:border-purple-600 transition-all">
                <?php foreach ($sessions as $s): ?>
                  <option value="<?= (int) $s['id'] ?>">
                    <?= htmlspecialchars($s['name']) ?> (<?= htmlspecialchars($s['phone_number'] ?? 'Tidak terhubung') ?>) -
                    [<?= strtoupper($s['status']) ?>]
                  </option>
                <?php endforeach; ?>
              </select>
            ...
            <!-- Recipient Number -->
            <div>
              <label class="block text-xs font-semibold text-gray-700 mb-1.5">Nomor Penerima</label>
              <input type="text" name="recipient" required placeholder="Contoh: 628123456789"
                class="w-full bg-gray-50 border border-gray-200 rounded-xl px-3 py-2 text-xs font-medium text-gray-800 focus:outline-none focus:ring-2 focus:ring-purple-500/20 focus:border-purple-600 transition-all" />
            </div>

            <!-- Message Type Selector -->
            <div>
              <label class="block text-xs font-semibold text-gray-700 mb-1.5">Jenis Pesan</label>
              <select name="message_type" id="messageTypeSelect" required onchange="toggleMediaFields()"
                class="w-full bg-gray-50 border border-gray-200 rounded-xl px-3 py-2 text-xs font-medium text-gray-800 focus:outline-none focus:ring-2 focus:ring-purple-500/20 focus:border-purple-600 transition-all">
            ...
            <div id="mediaUrlGroup" class="hidden">
              <label class="block text-xs font-semibold text-gray-700 mb-1.5">URL Media / File</label>
              <input type="url" name="media_url" id="mediaUrlInput" placeholder="https://example.com/file.jpg"
                class="w-full bg-gray-50 border border-gray-200 rounded-xl px-3 py-2 text-xs font-medium text-gray-800 focus:outline-none focus:ring-2 focus:ring-purple-500/20 focus:border-purple-600 transition-all" />
            ...
            <div>
              <label id="messageTextLabel" class="block text-xs font-semibold text-gray-700 mb-1.5">Isi Pesan</label>
              <textarea name="message_text" rows="4" placeholder="Tulis pesan Anda di sini..."
                class="w-full bg-gray-50 border border-gray-200 rounded-xl px-3 py-2 text-xs font-medium text-gray-800 focus:outline-none focus:ring-2 focus:ring-purple-500/20 focus:border-purple-600 transition-all resize-none"></textarea>
            </div>

            <button type="submit"
              class="w-full bg-purple-600 hover:bg-purple-700 text-white font-bold text-xs py-2.5 px-4 rounded-xl shadow-xs transition-all flex items-center justify-center gap-2">
      ...
      <div class="bg-white rounded-2xl border border-gray-100 p-6 shadow-xs">
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 mb-5 border-b border-gray-100">
          ...
          <form method="GET" action="<?= url('/messages') ?>" class="flex flex-wrap items-center gap-2">
            ...
            <select name="session_id" onchange="this.form.submit()"
              class="bg-gray-50 border border-gray-200 rounded-xl px-2.5 py-1.5 text-xs font-medium text-gray-700 focus:outline-none focus:border-purple-600">
            ...
            <!-- Search Query -->
            <div class="relative">
              <input type="text" name="q" value="<?= htmlspecialchars($searchQuery) ?>" placeholder="Cari nomor/isi..."
                class="bg-gray-50 border border-gray-200 rounded-xl pl-2.5 pr-7 py-1.5 text-xs font-medium text-gray-700 focus:outline-none focus:border-purple-600 w-32 sm:w-40" />
            ...
            <?php if ($selectedSessionId || $selectedType || $searchQuery): ?>
              <a href="<?= url('/messages') ?>" class="text-xs font-semibold text-rose-500 hover:underline px-1"
        ...
        <?php if (empty($messages)): ?>
          <div class="text-center py-12">
            <div
              class="w-10 h-10 rounded-xl bg-gray-50 border border-gray-200 text-gray-400 flex items-center justify-center mx-auto mb-3">
                ...
                <?php foreach ($messages as $msg): ?>
                  ...
                  <tr class="hover:bg-gray-50/50 transition-colors">
                    <td class="py-3 px-3 font-semibold text-gray-900 whitespace-nowrap">
                      <span
                        class="inline-flex items-center gap-1.5 bg-gray-100 px-2 py-0.5 rounded-md text-[11px] font-semibold text-gray-700 border border-gray-200">
                    ...
                    <td class="py-3 px-3 text-gray-600 max-w-xs truncate">
                      <p class="truncate" title="<?= htmlspecialchars((string) $text) ?>">
                        <?= htmlspecialchars((string) $text) ?>
                      </p>
                      <?php if ($fileUrl): ?>
                        <a href="<?= htmlspecialchars($fileUrl) ?>" target="_blank"
                          class="text-[10px] text-purple-600 hover:underline flex items-center gap-1 mt-0.5">
